...
export function readPaths(options: WorkflowTaskOptions | undefined): readonly string[] {
...
export function readPathEndsWith(options: WorkflowTaskOptions | undefined, suffix: string): boolean {
	const normalizedSuffix = normalizePathSeparators(suffix);
	return readPaths(options).some((path) => normalizePathSeparators(path).endsWith(normalizedSuffix));
}

export function assertStringOutput(output: WorkflowTaskOptions["output"] | undefined): asserts output is string {
...
export function assertOutputTypes(
	...
	for (const [key, type] of Object.entries(expected)) {
		...
		assert.ok(fieldDescription(outputs?.[key]).length > 0, `expected output description for ${key}`);
	}
}
