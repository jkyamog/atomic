...
function seedNested(
	...
	for (const stageId of stageIds) {
		targetStore.recordStageStart(childId, {
			id: stageId,
			name: stageId,
			status: "running",
			parentIds: [],
...
async function startAnsweredQuitSyntheticPrompt(runId: string): Promise<InMemoryDurableBackend> {
	...
	while (Date.now() < deadline) {
		promptStage = singletonStore
			.runs()
			.find((run) => run.id === runId)
			?.stages.find((stage) => stage.pendingPrompt !== undefined);
	...
	assert.equal((await quitRun(runId)).ok, true);
	assert.equal(
		singletonStore.resolveStagePendingPrompt(runId, promptStage!.id, promptStage!.pendingPrompt!.id, "paused-answer"),
		true,
	);
...
describe("post-commit quit and nested resume coherence", () => {
	test("quit records a fulfilled conforming pause handle in the Store before returning snapshots", async () => {
		...
		if (!result.ok) return;
		assert.deepEqual(
			result.paused.map((stage) => [stage.id, stage.status]),
			[["stage", "paused"]],
		);
	...
	test.sequential("tool-targeted nested progress then slash root retry resumes only the remaining descendant", async () => {
		...
		const targeted = await toolHandler()(
			{ action: "resume", runId: rootId, stageId: "first-child-stage" },
			{} as never,
		);
	...
	test.sequential("slash resume reports terminal completion acknowledged by a paused control", async () => {
		...
		singletonStore.recordStageStart(runId, {
			id: stageId,
			name: stageId,
			status: "running",
			parentIds: [],
		...
		stageControlRegistry.register(
			handle({
				...
				resume: async () => {
					...
					singletonStore.recordStageEnd(runId, {
						...stage!,
						status: "completed",
						endedAt: 3,
						durationMs: 2,
	...
	test.sequential("slash quit, held synthetic prompt answer, and resume reports one truthful acknowledgment", async () => {
		...
		await workflowCmd.options.handler(`quit ${runId}`, ctx);
		assert.equal(
			singletonStore.resolveStagePendingPrompt(runId, prompt!.stageId, prompt!.promptId, "held-answer"),
			true,
		);
		await sleep(10);
		assert.equal(singletonStore.runs().find((run) => run.id === runId)?.status, "paused");
